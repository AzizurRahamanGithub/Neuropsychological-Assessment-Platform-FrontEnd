...
export const BAARS_IV_ATTUALE_SELF: QuestionnaireDef = {
  ...
  instruction:
    "Per ognuna delle seguenti affermazioni, per favore selezioni l'opzione che meglio descrive la frequenza del Suo comportamento NEGLI ULTIMI 6 MESI.",
  questions,
};
...
function textToString(raw: any): string {
  ...
  if (typeof raw === "object") {
    const val =
      (raw as any).value ?? (raw as any).id ?? (raw as any).label ?? "";
...
export function computeBAARSIVSelf(
  ...
  const iperattivitaImpulsivitaSintomiComputed =
    iper_sint_10_14 + impu_sint_15_18;
  ...
  const totalPunteggio = classify(
    totalScoreComputed,
    norms.totale_adhd_punteggio,
  );

  const disaSintomi = classify(diso_sint_1_9, norms.disattenzione_sintomi);
  const iperattivita_ImpulsivitaiSintomi = classify(
    iperattivitaImpulsivitaSintomiComputed,
    norms.iperattivita_impulsivita_sintomi,
  );
  const totalSintomi = classify(
    totalSymptomsComputed,
    norms.totale_adhd_sintomi,
  );
  ...
  return {
    ...
    "Iperattività punteggio STAT": iperPunteggio.stat,
    "Iperattività punteggio ESITO": iperPunteggio.esito,

    "n° sintomi iperattività/impulsività PG": String(
      iperattivitaImpulsivitaSintomiComputed,
    ),
    "n° sintomi iperattività/impulsività STAT":
      iperattivita_ImpulsivitaiSintomi.stat,
    "n° sintomi iperattività/impulsività ESITO":
      iperattivita_ImpulsivitaiSintomi.esito,
